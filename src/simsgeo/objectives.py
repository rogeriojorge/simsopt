...
class CurveLength:
    ...
    def __init__(self, curve):
        self.curve = curve
        self.thisgrad = jit(
            lambda gammadash: grad(curve_length_pure)(gammadash))
    ...
    def dJ(self):
        return self.curve.dgammadash_by_dcoeff_vjp(
            self.thisgrad(self.curve.gammadash()))
...
def Lp_curvature_pure(kappa, gammadash, p, desired_kappa):
    ...
    return (1. / p) * jnp.mean(
        jnp.maximum(kappa - desired_kappa, 0) ** p * arc_length)
...
class LpCurveCurvature:
    ...
    def __init__(self, curve, p, desired_length=None):
        ...
        if desired_length is None:
            ...
            radius = desired_length / (2 * jnp.pi)
            self.desired_kappa = 1 / radius

        self.J_jax = jit(
            lambda kappa, gammadash: Lp_curvature_pure(kappa, gammadash, p,
                                                       self.desired_kappa))
        self.thisgrad0 = jit(
            lambda kappa, gammadash: grad(self.J_jax, argnums=0)(kappa,
                                                                 gammadash))
        self.thisgrad1 = jit(
            lambda kappa, gammadash: grad(self.J_jax, argnums=1)(kappa,
                                                                 gammadash))
    ...
    def dJ(self):
        ...
        return self.curve.dkappa_by_dcoeff_vjp(
            grad0) + self.curve.dgammadash_by_dcoeff_vjp(grad1)
...
class LpCurveTorsion:
    ...
    def __init__(self, curve, p):
        ...
        self.J_jax = jit(
            lambda torsion, gammadash: Lp_torsion_pure(torsion, gammadash, p))
        self.thisgrad0 = jit(
            lambda torsion, gammadash: grad(self.J_jax, argnums=0)(torsion,
                                                                   gammadash))
        self.thisgrad1 = jit(
            lambda torsion, gammadash: grad(self.J_jax, argnums=1)(torsion,
                                                                   gammadash))
...
class MinimumDistance:
    ...
    def __init__(self, curves, minimum_distance):
        ...
        self.J_jax = jit(
            lambda gamma1, l1, gamma2, l2: distance_pure(
    ...
    def dJ(self):
        dgamma_by_dcoeff_vjp_vecs = [None] * len(self.curves)
        dgammadash_by_dcoeff_vjp_vecs = [None] * len(self.curves)
        for i in range(len(self.curves)):
            gamma1 = self.curves[i].gamma()
            l1 = self.curves[i].gammadash()
            for j in range(i):
        ...
        res = [
            ...
            self.curves[i].dgammadash_by_dcoeff_vjp(
                dgammadash_by_dcoeff_vjp_vecs[i]) for i in
