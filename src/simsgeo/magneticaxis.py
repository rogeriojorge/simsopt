...
def stelleratorsymmetriccylindricalfouriercurve_pure(dofs, quadpoints, order,
                                                     nfp):
    coefficients0 = dofs[:(order + 1)]
    coefficients1 = dofs[(order + 1):]
    gamma = jnp.zeros((len(quadpoints), 3))
    for i in range(order + 1):
        gamma = index_add(gamma, index[:, 0], coefficients0[i] * jnp.cos(
            nfp * 2 * pi * i * quadpoints) * jnp.cos(2 * pi * quadpoints))
        gamma = index_add(gamma, index[:, 1], coefficients0[i] * jnp.cos(
            nfp * 2 * pi * i * quadpoints) * jnp.sin(2 * pi * quadpoints))
    for i in range(1, order + 1):
        gamma = index_add(gamma, index[:, 2], coefficients1[i - 1] * jnp.sin(
            nfp * 2 * pi * i * quadpoints))
    return gamma
...
class StelleratorSymmetricCylindricalFourierCurve(JaxCurve):
    ...
    def __init__(self, quadpoints, order, nfp):
        ...
        def pure(dofs, points):
            return stelleratorsymmetriccylindricalfouriercurve_pure(
                dofs, points, order, nfp)
