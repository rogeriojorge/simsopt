...
incremental_arclengthgrad0 = jit(
    lambda d1gamma, v: vjp(incremental_arclength_pure, d1gamma)[1](v))


@jit
def kappa_pure(d1gamma, d2gamma):
    return jnp.linalg.norm(jnp.cross(d1gamma, d2gamma),
                           axis=1) / jnp.linalg.norm(d1gamma, axis=1) ** 3


kappavjp0 = jit(lambda d1gamma, d2gamma, v:
                vjp(lambda d1g: kappa_pure(d1g, d2gamma), d1gamma)[1](v))
kappavjp1 = jit(lambda d1gamma, d2gamma, v:
                vjp(lambda d2g: kappa_pure(d1gamma, d2g), d2gamma)[1](v))
kappagrad0 = jit(lambda d1gamma, d2gamma:
                 jacfwd(lambda d1g: kappa_pure(d1g, d2gamma))(d1gamma))
kappagrad1 = jit(lambda d1gamma, d2gamma:
                 jacfwd(lambda d2g: kappa_pure(d1gamma, d2g))(d2gamma))


@jit
def torsion_pure(d1gamma, d2gamma, d3gamma):
    return jnp.sum(jnp.cross(d1gamma, d2gamma, axis=1) * d3gamma,
                   axis=1) / jnp.sum(jnp.cross(d1gamma, d2gamma, axis=1) ** 2,
                                     axis=1)


torsiongrad0 = jit(lambda d1gamma, d2gamma, d3gamma, v:
                   vjp(lambda d1g: torsion_pure(d1g, d2gamma, d3gamma),
                       d1gamma)[1](v))
torsiongrad1 = jit(lambda d1gamma, d2gamma, d3gamma, v:
                   vjp(lambda d2g: torsion_pure(d1gamma, d2g, d3gamma),
                       d2gamma)[1](v))
torsiongrad2 = jit(lambda d1gamma, d2gamma, d3gamma, v:
                   vjp(lambda d3g: torsion_pure(d1gamma, d2gamma, d3g),
                       d3gamma)[1](v))


class Curve():
    def __init__(self):
        self.dincremental_arclength_by_dcoeff_vjp_jax = jit(lambda x, v: vjp(
            lambda d: incremental_arclength_pure(self.gammadash_jax(d)), x)[1](
            v))
        self.dincremental_arclength_by_dcoeff_jax = jit(
            jacfwd(lambda d: incremental_arclength_pure(self.gammadash_jax(d))))
        self.dependencies = []
    ...
    def plot(self, ax=None, show=True, plot_derivative=False, closed_loop=True,
        ...
        if plot_derivative:
            ax.quiver(rep(gamma[:, 0]), rep(gamma[:, 1]), rep(gamma[:, 2]),
                      0.1 * rep(gammadash[:, 0]),
                      0.1 * rep(gammadash[:, 1]), 0.1 * rep(gammadash[:, 2]),
    ...
    def kappa_impl(self, kappa):
        kappa[:] = np.asarray(
            kappa_pure(self.gammadash(), self.gammadashdash()))
    ...
    def dkappa_by_dcoeff_impl(self, dkappa_by_dcoeff):
        ...
        dkappa_by_dcoeff[:, :] = (1 / (denominator ** 3 * norm(numerator)))[:,
                                 None] * np.sum(numerator[:, :, None] * (
                np.cross(dgamma_by_dphidcoeff[:, :, :],
                         dgamma_by_dphidphi[:, :, None], axis=1) +
                np.cross(dgamma_by_dphi[:, :, None],
                         dgamma_by_dphidphidcoeff[:, :, :], axis=1)), axis=1) \
                                 - (norm(numerator) * 3 / denominator ** 5)[:,
                                   None] * np.sum(
    ...
    def dtorsion_by_dcoeff_impl(self, dtorsion_by_dcoeff):
        ...
        dtorsion_by_dcoeff[:, :] = (
                                           np.sum(np.cross(d1gamma, d2gamma,
                                                           axis=1)[:, :,
                                                  None] * d3gammadcoeff, axis=1)
                                           + np.sum((np.cross(d1gammadcoeff,
                                                              d2gamma[:, :,
                                                              None],
                                                              axis=1) + np.cross(
                                       d1gamma[:, :, None], d2gammadcoeff,
                                       axis=1)) * d3gamma[:, :, None], axis=1)
                                   ) / np.sum(
            np.cross(d1gamma, d2gamma, axis=1) ** 2, axis=1)[:, None]
        dtorsion_by_dcoeff[:, :] -= np.sum(
            np.cross(d1gamma, d2gamma, axis=1) * d3gamma, axis=1)[:,
                                    None] * np.sum(
            2 * np.cross(d1gamma, d2gamma, axis=1)[:, :, None] * (
                        np.cross(d1gammadcoeff, d2gamma[:, :, None],
                                 axis=1) + np.cross(d1gamma[:, :, None],
                                                    d2gammadcoeff, axis=1)),
            axis=1) / np.sum(np.cross(d1gamma, d2gamma, axis=1) ** 2, axis=1)[:,
                      None] ** 2
    ...
    def dtorsion_by_dcoeff_vjp(self, v):
        return self.dgammadash_by_dcoeff_vjp(
            torsiongrad0(self.gammadash(), self.gammadashdash(),
                         self.gammadashdashdash(), v)) \
               + self.dgammadashdash_by_dcoeff_vjp(
            torsiongrad1(self.gammadash(), self.gammadashdash(),
                         self.gammadashdashdash(), v)) \
               + self.dgammadashdashdash_by_dcoeff_vjp(
            torsiongrad2(self.gammadash(), self.gammadashdash(),
                         self.gammadashdashdash(), v))
    ...
    def kappadash(self):
        ...
        dkappa_by_dphi[:] = inner(cross(dgamma, d2gamma),
                                  cross(dgamma, d3gamma)) / (
                                        norm(cross(dgamma, d2gamma)) * norm(
                                    dgamma) ** 3) \
                            - 3 * inner(dgamma, d2gamma) * norm(
            cross(dgamma, d2gamma)) / norm(dgamma) ** 5
    ...
    def dfrenet_frame_by_dcoeff(self):
        ...
        np.zeros((N, 3, self.num_dofs())), np.zeros((N, 3, self.num_dofs())),
        np.zeros((N, 3, self.num_dofs())))
        t, n, b = self.frenet_frame()

        dt_by_dcoeff[:, :, :] = -(dl_by_dcoeff[:, None, :] / l[:, None,
                                                             None] ** 2) * dgamma_by_dphi[
                                                                           :, :,
                                                                           None] \
        ...
        tdash = (1. / l[:, None]) ** 2 * (
                ...
                - (inner(dgamma_by_dphi, d2gamma_by_dphidphi) / l)[:,
                  None] * dgamma_by_dphi
        )

        dtdash_by_dcoeff = (-2 * dl_by_dcoeff[:, None, :] / l[:, None,
                                                            None] ** 3) * (l[:,
                                                                           None] * d2gamma_by_dphidphi - (
                                                                                                                     inner(
                                                                                                                         dgamma_by_dphi,
                                                                                                                         d2gamma_by_dphidphi) / l)[
                                                                                                         :,
                                                                                                         None] * dgamma_by_dphi)[
                                                                          :, :,
                                                                          None] \
                           + (1. / l[:, None, None]) ** 2 * (
                                   dl_by_dcoeff[:, None,
                                   :] * d2gamma_by_dphidphi[:, :, None] + l[:,
                                                                          None,
                                                                          None] * d3gamma_by_dphidphidcoeff
                                   - (inner(d2gamma_by_dphidcoeff,
                                            d2gamma_by_dphidphi[:, :, None])[:,
                                      None, :] / l[:, None,
                                                 None]) * dgamma_by_dphi[:, :,
                                                          None]
                                   - (inner(dgamma_by_dphi[:, :, None],
                                            d3gamma_by_dphidphidcoeff)[:, None,
                                      :] / l[:, None, None]) * dgamma_by_dphi[:,
                                                               :, None]
                                   + (inner(dgamma_by_dphi,
                                            d2gamma_by_dphidphi)[:, None,
                                      None] * dl_by_dcoeff[:, None, :] / l[:,
                                                                         None,
                                                                         None] ** 2) * dgamma_by_dphi[
                                                                                       :,
                                                                                       :,
                                                                                       None]
                                   - (inner(dgamma_by_dphi,
                                            d2gamma_by_dphidphi) / l)[:, None,
                                     None] * d2gamma_by_dphidcoeff
                           )
        dn_by_dcoeff[:, :, :] = (1. / norm(tdash))[:, None,
                                None] * dtdash_by_dcoeff \
                                - (inner(tdash[:, :, None], dtdash_by_dcoeff)[:,
                                   None, :] / inner(tdash, tdash)[:, None,
                                              None] ** 1.5) * tdash[:, :, None]

        db_by_dcoeff[:, :, :] = np.cross(dt_by_dcoeff, n[:, :, None],
                                         axis=1) + np.cross(t[:, :, None],
                                                            dn_by_dcoeff,
                                                            axis=1)
    ...
    def dkappadash_by_dcoeff(self):
        ...
        for i in range(self.num_dofs()):
            ...
            dkappadash_by_dcoeff[:, i] = (
                                                 +inner(
                                                     d1coeff_x_d2 + d1_x_d2coeff,
                                                     d1_x_d3)
                                                 + inner(d1_x_d2,
                                                         d1coeff_x_d3 + d1_x_d3coeff)
                                         ...
                                         - inner(d1_x_d2, d1_x_d3) * (
                                             (
                                                     inner(
                                                         d1coeff_x_d2 + d1_x_d2coeff,
                                                         d1_x_d2) / (norm(
                                                 d1_x_d2) ** 3 * normdgamma ** 3)
                                                     + 3 * inner(dgamma,
                                                                 dgamma_dcoeff) / (
                                                                 norm(
                                                                     d1_x_d2) * normdgamma ** 5)
                                             )
                                         ) \
                                         - 3 * (
                                                 + (
                                                             d1coeff_dot_d2 + d1_dot_d2coeff) * norm(
                                             d1_x_d2) / normdgamma ** 5
                                                 + d1_dot_d2 * inner(
                                             d1coeff_x_d2 + d1_x_d2coeff,
                                             d1_x_d2) / (norm(
                                             d1_x_d2) * normdgamma ** 5)
                                                 - 5 * d1_dot_d2 * norm(
                                             d1_x_d2) * d1_dot_d1coeff / normdgamma ** 7
                                         )
...
class JaxCurve(sgpp.Curve, Curve):
    def __init__(self, quadpoints, gamma_pure):
        ...
        self.dgamma_by_dcoeff_vjp_jax = jit(
            lambda x, v: vjp(self.gamma_jax, x)[1](v))

        self.gammadash_pure = lambda x, q: \
        jvp(lambda p: self.gamma_pure(x, p), (q,), (ones,))[1]
        ...
        self.dgammadash_by_dcoeff_vjp_jax = jit(
            lambda x, v: vjp(self.gammadash_jax, x)[1](v))

        self.gammadashdash_pure = lambda x, q: \
        jvp(lambda p: self.gammadash_pure(x, p), (q,), (ones,))[1]
        self.gammadashdash_jax = jit(
            lambda x: self.gammadashdash_pure(x, points))
        self.dgammadashdash_by_dcoeff_jax = jit(jacfwd(self.gammadashdash_jax))
        self.dgammadashdash_by_dcoeff_vjp_jax = jit(
            lambda x, v: vjp(self.gammadashdash_jax, x)[1](v))

        self.gammadashdashdash_pure = lambda x, q: \
        jvp(lambda p: self.gammadashdash_pure(x, p), (q,), (ones,))[1]
        self.gammadashdashdash_jax = jit(
            lambda x: self.gammadashdashdash_pure(x, points))
        self.dgammadashdashdash_by_dcoeff_jax = jit(
            jacfwd(self.gammadashdashdash_jax))
        self.dgammadashdashdash_by_dcoeff_vjp_jax = jit(
            lambda x, v: vjp(self.gammadashdashdash_jax, x)[1](v))

        self.dkappa_by_dcoeff_vjp_jax = jit(lambda x, v: vjp(
            lambda d: kappa_pure(self.gammadash_jax(d),
                                 self.gammadashdash_jax(d)), x)[1](v))
        self.dkappa_by_dcoeff_jax = jit(jacfwd(
            lambda d: kappa_pure(self.gammadash_jax(d),
                                 self.gammadashdash_jax(d))))

        self.dtorsion_by_dcoeff_vjp_jax = jit(lambda x, v: vjp(
            lambda d: torsion_pure(self.gammadash_jax(d),
                                   self.gammadashdash_jax(d),
                                   self.gammadashdashdash_jax(d)), x)[1](v))
        self.dtorsion_by_dcoeff_jax = jit(jacfwd(
            lambda d: torsion_pure(self.gammadash_jax(d),
                                   self.gammadashdash_jax(d),
    ...
    def dgammadashdashdash_by_dcoeff_impl(self, dgammadashdashdash_by_dcoeff):
        ...
        :] = self.dgammadashdashdash_by_dcoeff_jax(self.get_dofs())
...
class RotatedCurve(sgpp.Curve, Curve):
    ...
    def dgammadash_by_dcoeff_impl(self, dgammadash_by_dcoeff):
        ...
        :] = self.rotmatT @ self.curve.dgammadash_by_dcoeff()
