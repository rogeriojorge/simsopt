...
class CurveRZFourier : public Curve<Array> {
 ...
 public:
  ...
  CurveRZFourier(int _numquadpoints, int _order, int _nfp, bool _stellsym)
      : Curve<Array>(_numquadpoints), order(_order), nfp(_nfp), stellsym(_stellsym) {
  ...
  CurveRZFourier(vector<double> _quadpoints, int _order, int _nfp, bool _stellsym)
      : Curve<Array>(_quadpoints), order(_order), nfp(_nfp), stellsym(_stellsym) {
    rc = xt::zeros<double>({order + 1});
    rs = xt::zeros<double>({order});
    zc = xt::zeros<double>({order + 1});
    zs = xt::zeros<double>({order});
  ...
  CurveRZFourier(Array _quadpoints, int _order, int _nfp, bool _stellsym)
      : Curve<Array>(_quadpoints), order(_order), nfp(_nfp), stellsym(_stellsym) {
    rc = xt::zeros<double>({order + 1});
    rs = xt::zeros<double>({order});
    zc = xt::zeros<double>({order + 1});
  ...
  void gammadashdash_impl(Array& data) override {
    ...
    for (int k = 0; k < numquadpoints; ++k) {
      double phi = 2 * M_PI * quadpoints[k];
      for (int i = 0; i < order + 1; ++i) {
        data(k, 0) += rc[i] * (+2 * (nfp * i) * sin(nfp * i * phi) * sin(phi) -
                               (pow(nfp * i, 2) + 1) * cos(nfp * i * phi) * cos(phi));
        data(k, 1) += rc[i] * (-2 * (nfp * i) * sin(nfp * i * phi) * cos(phi) -
    ...
    if (!stellsym) {
      for (int k = 0; k < numquadpoints; ++k) {
        double phi = 2 * M_PI * quadpoints[k];
        for (int i = 1; i < order + 1; ++i) {
          data(k, 0) += rs[i - 1] * (-(pow(nfp * i, 2) + 1) * sin(nfp * i * phi) * cos(phi) -
                                     2 * (i * nfp) * cos(nfp * i * phi) * sin(phi));
          data(k, 1) += rs[i - 1] * (-(pow(nfp * i, 2) + 1) * sin(nfp * i * phi) * sin(phi) +
  ...
  void gammadashdashdash_impl(Array& data) override {
    ...
    for (int k = 0; k < numquadpoints; ++k) {
      double phi = 2 * M_PI * quadpoints[k];
      for (int i = 0; i < order + 1; ++i) {
        data(k, 0) += rc[i] * (+(3 * pow(nfp * i, 2) + 1) * cos(nfp * i * phi) * sin(phi) +
                               (pow(nfp * i, 2) + 3) * (nfp * i) * sin(nfp * i * phi) * cos(phi));
        data(k, 1) += rc[i] * (+(pow(nfp * i, 2) + 3) * (nfp * i) * sin(nfp * i * phi) * sin(phi) -
    ...
    if (!stellsym) {
      for (int k = 0; k < numquadpoints; ++k) {
        double phi = 2 * M_PI * quadpoints[k];
        for (int i = 1; i < order + 1; ++i) {
          data(k, 0) += rs[i - 1] * (-(pow(nfp * i, 2) + 3) * (nfp * i) * cos(nfp * i * phi) * cos(phi) +
                                     (3 * pow(nfp * i, 2) + 1) * sin(nfp * i * phi) * sin(phi));
          data(k, 1) += rs[i - 1] * (-(pow(nfp * i, 2) + 3) * (nfp * i) * cos(nfp * i * phi) * sin(phi) -
  ...
  void dgammadashdash_by_dcoeff_impl(Array& data) override {
    for (int k = 0; k < numquadpoints; ++k) {
      ...
      for (int i = 0; i < order + 1; ++i) {
        data(k, 0, counter) =
            (+2 * (nfp * i) * sin(nfp * i * phi) * sin(phi) - (pow(nfp * i, 2) + 1) * cos(nfp * i * phi) * cos(phi));
        data(k, 1, counter) =
            (-2 * (nfp * i) * sin(nfp * i * phi) * cos(phi) - (pow(nfp * i, 2) + 1) * cos(nfp * i * phi) * sin(phi));
        counter++;
      }
      if (!stellsym) {
        for (int i = 1; i < order + 1; ++i) {
          data(k, 0, counter) =
              (-(pow(nfp * i, 2) + 1) * sin(nfp * i * phi) * cos(phi) - 2 * (i * nfp) * cos(nfp * i * phi) * sin(phi));
          data(k, 1, counter) =
              (-(pow(nfp * i, 2) + 1) * sin(nfp * i * phi) * sin(phi) + 2 * (i * nfp) * cos(nfp * i * phi) * cos(phi));
          counter++;
        }
        for (int i = 0; i < order + 1; ++i) {
  ...
  void dgammadashdashdash_by_dcoeff_impl(Array& data) override {
    for (int k = 0; k < numquadpoints; ++k) {
      ...
      int counter = 0;
      for (int i = 0; i < order + 1; ++i) {
        data(k, 0, counter) = (+(3 * pow(nfp * i, 2) + 1) * cos(nfp * i * phi) * sin(phi) +
                               (pow(nfp * i, 2) + 3) * (nfp * i) * sin(nfp * i * phi) * cos(phi));
        data(k, 1, counter) = (+(pow(nfp * i, 2) + 3) * (nfp * i) * sin(nfp * i * phi) * sin(phi) -
                               (3 * pow(nfp * i, 2) + 1) * cos(nfp * i * phi) * cos(phi));
        counter++;
      }
      if (!stellsym) {
        for (int i = 1; i < order + 1; ++i) {
          data(k, 0, counter) = (-(pow(nfp * i, 2) + 3) * (nfp * i) * cos(nfp * i * phi) * cos(phi) +
                                 (3 * pow(nfp * i, 2) + 1) * sin(nfp * i * phi) * sin(phi));
          data(k, 1, counter) = (-(pow(nfp * i, 2) + 3) * (nfp * i) * cos(nfp * i * phi) * sin(phi) -
                                 (3 * pow(nfp * i, 2) + 1) * sin(nfp * i * phi) * cos(phi));
          counter++;
        }
        for (int i = 0; i < order + 1; ++i) {
