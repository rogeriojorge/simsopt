...
inline simd_t inner(const Vec3dSimd& a, const Vec3dSimd& b) { return a.x * b.x + a.y * b.y + a.z * b.z; }

inline simd_t inner(const Vec3d& b, const Vec3dSimd& a) { return a.x * b[0] + a.y * b[1] + a.z * b[2]; }

inline simd_t inner(const Vec3dSimd& a, const Vec3d& b) { return a.x * b[0] + a.y * b[1] + a.z * b[2]; }
...
inline double inner(const Vec3d& a, const Vec3d& b) { return a.dot(b); }

inline Vec3d cross(const Vec3d& a, const Vec3d& b) { return a.cross(b); }

inline double norm(const Vec3d& a) { return a.norm(); }

inline Vec3dSimd cross(Vec3dSimd& a, Vec3dSimd& b) {
  return Vec3dSimd(xsimd::fms(a.y, b.z, a.z * b.y), xsimd::fms(a.z, b.x, a.x * b.z), xsimd::fms(a.x, b.y, a.y * b.x));
}
...
inline simd_t normsq(Vec3dSimd& a) { return a.x * a.x + a.y * a.y + a.z * a.z; }
...
template <class T, int derivs>
void biot_savart_vjp_kernel(vector_type& pointsx, vector_type& pointsy, vector_type& pointsz, T& gamma,
                            T& dgamma_by_dphi, T& v, T& res_gamma, T& res_dgamma_by_dphi, T& vgrad, T& res_grad_gamma,
                            T& res_grad_dgamma_by_dphi);

void biot_savart_vjp(Array& points, vector<Array>& gammas, vector<Array>& dgamma_by_dphis, vector<double>& currents,
                     Array& v, Array& vgrad, vector<Array>& dgamma_by_dcoeffs, vector<Array>& d2gamma_by_dphidcoeffs,
                     vector<Array>& res_B, vector<Array>& res_dB);
