...
class SurfaceXYZFourier : public Surface<Array> {
 ...
 public:
  ...
  SurfaceXYZFourier(int _mpol, int _ntor, int _nfp, bool _stellsym, vector<double> _quadpoints_phi,
                    vector<double> _quadpoints_theta)
  ...
  SurfaceXYZFourier(int _mpol, int _ntor, int _nfp, bool _stellsym, int _numquadpoints_phi, int _numquadpoints_theta)
      : Surface<Array>(_numquadpoints_phi, _numquadpoints_theta),
        mpol(_mpol),
        ntor(_ntor),
        nfp(_nfp),
        stellsym(_stellsym) {
      ...
      for (int k2 = 0; k2 < numquadpoints_theta; ++k2) {
        ...
        for (int m = 0; m <= mpol; ++m) {
          for (int i = 0; i < 2 * ntor + 1; ++i) {
            int n = i - ntor;
            double xhat = get_coeff(0, true, m, i) * cos(m * theta - n * nfp * phi) +
                          get_coeff(0, false, m, i) * sin(m * theta - n * nfp * phi);
            double yhat = get_coeff(1, true, m, i) * cos(m * theta - n * nfp * phi) +
                          get_coeff(1, false, m, i) * sin(m * theta - n * nfp * phi);
            double x = xhat * cos(phi) - yhat * sin(phi);
            double y = xhat * sin(phi) + yhat * cos(phi);
            double z = get_coeff(2, true, m, i) * cos(m * theta - n * nfp * phi) +
                       get_coeff(2, false, m, i) * sin(m * theta - n * nfp * phi);
            data(k1, k2, 0) += x;
            data(k1, k2, 1) += y;
            data(k1, k2, 2) += z;
          }
      ...
      for (int k2 = 0; k2 < numquadpoints_theta; ++k2) {
        ...
        for (int m = 0; m <= mpol; ++m) {
          for (int i = 0; i < 2 * ntor + 1; ++i) {
            ...
            double yhat = get_coeff(1, true, m, i) * cos(m * theta - n * nfp * phi) +
                          get_coeff(1, false, m, i) * sin(m * theta - n * nfp * phi);
            double xhatdash = get_coeff(0, true, m, i) * (n * nfp) * sin(m * theta - n * nfp * phi) +
                              get_coeff(0, false, m, i) * (-n * nfp) * cos(m * theta - n * nfp * phi);
            double yhatdash = get_coeff(1, true, m, i) * (n * nfp) * sin(m * theta - n * nfp * phi) +
                              get_coeff(1, false, m, i) * (-n * nfp) * cos(m * theta - n * nfp * phi);
            ...
            double zdash = get_coeff(2, true, m, i) * (n * nfp) * sin(m * theta - n * nfp * phi) +
                           get_coeff(2, false, m, i) * (-n * nfp) * cos(m * theta - n * nfp * phi);
    ...
    for (int k1 = 0; k1 < numquadpoints_phi; ++k1) {
      ...
      for (int k2 = 0; k2 < numquadpoints_theta; ++k2) {
        ...
        for (int m = 0; m <= mpol; ++m) {
          for (int i = 0; i < 2 * ntor + 1; ++i) {
            int n = i - ntor;
            double xhatdash = get_coeff(0, true, m, i) * (-m) * sin(m * theta - n * nfp * phi) +
                              get_coeff(0, false, m, i) * m * cos(m * theta - n * nfp * phi);
            double yhatdash = get_coeff(1, true, m, i) * (-m) * sin(m * theta - n * nfp * phi) +
                              get_coeff(1, false, m, i) * m * cos(m * theta - n * nfp * phi);
            double xdash = xhatdash * cos(phi) - yhatdash * sin(phi);
            double ydash = xhatdash * sin(phi) + yhatdash * cos(phi);
            double zdash = get_coeff(2, true, m, i) * (-m) * sin(m * theta - n * nfp * phi) +
                           get_coeff(2, false, m, i) * m * cos(m * theta - n * nfp * phi);
            data(k1, k2, 0) += 2 * M_PI * xdash;
            data(k1, k2, 1) += 2 * M_PI * ydash;
            data(k1, k2, 2) += 2 * M_PI * zdash;
          }
        ...
        for (int d = 0; d < 3; ++d) {
          for (int m = 0; m <= mpol; ++m) {
            for (int n = -ntor; n <= ntor; ++n) {
              if (m == 0 && n < 0) continue;
              if (d == 0) {
                data(k1, k2, 0, counter) =
                    (n * nfp) * sin(m * theta - n * nfp * phi) * cos(phi) - cos(m * theta - n * nfp * phi) * sin(phi);
                data(k1, k2, 1, counter) =
                    (n * nfp) * sin(m * theta - n * nfp * phi) * sin(phi) + cos(m * theta - n * nfp * phi) * cos(phi);
              } else if (d == 1) {
                if (stellsym) continue;
                data(k1, k2, 0, counter) =
                    -(n * nfp) * sin(m * theta - n * nfp * phi) * sin(phi) - cos(m * theta - n * nfp * phi) * cos(phi);
          ...
          for (int m = 0; m <= mpol; ++m) {
            for (int n = -ntor; n <= ntor; ++n) {
              ...
              if (d == 0) {
                if (stellsym) continue;
                data(k1, k2, 0, counter) =
                    -(n * nfp) * cos(m * theta - n * nfp * phi) * cos(phi) - sin(m * theta - n * nfp * phi) * sin(phi);
                data(k1, k2, 1, counter) =
                    -(n * nfp) * cos(m * theta - n * nfp * phi) * sin(phi) + sin(m * theta - n * nfp * phi) * cos(phi);
              } else if (d == 1) {
                data(k1, k2, 0, counter) =
                    (n * nfp) * cos(m * theta - n * nfp * phi) * sin(phi) - sin(m * theta - n * nfp * phi) * cos(phi);
                data(k1, k2, 1, counter) =
