...
template <class CurveBase = PyCurve>
class PyCurveTrampoline : public CurveBase {
 public:
  using CurveBase::CurveBase;

  virtual int num_dofs() override { PYBIND11_OVERLOAD_PURE(int, CurveBase, num_dofs); }

  virtual void set_dofs_impl(const vector<double>& _dofs) override {
    PYBIND11_OVERLOAD_PURE(void, CurveBase, set_dofs_impl, _dofs);
  }

  virtual vector<double> get_dofs() override { PYBIND11_OVERLOAD_PURE(vector<double>, CurveBase, get_dofs); }
  ...
  virtual void gammadash_impl(PyArray& data) override { PYBIND11_OVERLOAD(void, CurveBase, gammadash_impl, data); }
  ...
  virtual void kappa_impl(PyArray& data) override { PYBIND11_OVERLOAD(void, CurveBase, kappa_impl, data); }

  virtual void dkappa_by_dcoeff_impl(PyArray& data) override {
    PYBIND11_OVERLOAD(void, CurveBase, dkappa_by_dcoeff_impl, data);
  }

  virtual void torsion_impl(PyArray& data) override { PYBIND11_OVERLOAD(void, CurveBase, torsion_impl, data); }
