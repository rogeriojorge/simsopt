...
class CurveXYZFourier : public Curve<Array> {
  /*
     ...
     x(phi) = \sum_{m=0}^{order} x_{c,m}cos(phi) + \sum_{m=1}^order x_{s,m}sin(phi)
     y(phi) = \sum_{m=0}^{order} y_{c,m}cos(phi) + \sum_{m=1}^order y_{s,m}sin(phi)
     z(phi) = \sum_{m=0}^{order} z_{c,m}cos(phi) + \sum_{m=1}^order z_{s,m}sin(phi)
 ...
 public:
  ...
  CurveXYZFourier(int _numquadpoints, int _order) : Curve<Array>(_numquadpoints), order(_order) {
    dofs = vector<vector<double>>{vector<double>(2 * order + 1, 0.), vector<double>(2 * order + 1, 0.),
  ...
  inline int num_dofs() override { return 3 * (2 * order + 1); }
  ...
  void gammadashdashdash_impl(Array& data) override {
    ...
    for (int k = 0; k < numquadpoints; ++k) {
      for (int i = 0; i < 3; ++i) {
        for (int j = 1; j < order + 1; ++j) {
          data(k, i) += -dofs[i][2 * j - 1] * (2 * M_PI * j) * (2 * M_PI * j) * (2 * M_PI * j) *
                        cos(2 * M_PI * j * quadpoints[k]);
          data(k, i) +=
              +dofs[i][2 * j] * (2 * M_PI * j) * (2 * M_PI * j) * (2 * M_PI * j) * sin(2 * M_PI * j * quadpoints[k]);
        }
      }
    }
  }
  ...
  void dgammadashdash_by_dcoeff_impl(Array& data) override {
    for (int k = 0; k < numquadpoints; ++k) {
      for (int i = 0; i < 3; ++i) {
        for (int j = 1; j < order + 1; ++j) {
          data(k, i, i * (2 * order + 1) + 2 * j - 1) =
              -(2 * M_PI * j) * (2 * M_PI * j) * sin(2 * M_PI * j * quadpoints[k]);
          data(k, i, i * (2 * order + 1) + 2 * j) =
              -(2 * M_PI * j) * (2 * M_PI * j) * cos(2 * M_PI * j * quadpoints[k]);
        }
      }
    }
  }
  ...
  void dgammadashdashdash_by_dcoeff_impl(Array& data) override {
    for (int k = 0; k < numquadpoints; ++k) {
      for (int i = 0; i < 3; ++i) {
        for (int j = 1; j < order + 1; ++j) {
          data(k, i, i * (2 * order + 1) + 2 * j - 1) =
              -(2 * M_PI * j) * (2 * M_PI * j) * (2 * M_PI * j) * cos(2 * M_PI * j * quadpoints[k]);
          data(k, i, i * (2 * order + 1) + 2 * j) =
              +(2 * M_PI * j) * (2 * M_PI * j) * (2 * M_PI * j) * sin(2 * M_PI * j * quadpoints[k]);
        }
      }
    }
  }
