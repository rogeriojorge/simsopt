...
void biot_savart_vjp_kernel(vector_type& pointsx, vector_type& pointsy, vector_type& pointsz, T& gamma,
    ...
    for (int j = 0; j < num_quad_points; ++j) {
      ...
      MYIF(derivs > 0) {
        ...
        for (int k = 0; k < 3; k++) {
          ...
          res_grad_gamma_add -=
              diff * (15. * diff[k] * inner(cross_dgamma_j_by_dphi_diff, vgrad_i[k]) * norm_diff_7_inv);
        }
  ...
  for (int i = num_points - num_points % simd_size; i < num_points; ++i) {
    ...
    auto vgrad_i = vector<Vec3d>{Vec3d::Zero(), Vec3d::Zero(), Vec3d::Zero()};
    for (int d = 0; d < 3; ++d) {
      v_i[d] = v(i, d);
      MYIF(derivs > 0) {
        for (int dd = 0; dd < 3; ++dd) {
...
template void biot_savart_vjp_kernel<xt::xarray<double>, 0>(vector_type&, vector_type&, vector_type&,
                                                            xt::xarray<double>&, xt::xarray<double>&,
                                                            xt::xarray<double>&, xt::xarray<double>&,
                                                            xt::xarray<double>&, xt::xarray<double>&,
                                                            xt::xarray<double>&, xt::xarray<double>&);
template void biot_savart_vjp_kernel<xt::xarray<double>, 1>(vector_type&, vector_type&, vector_type&,
                                                            xt::xarray<double>&, xt::xarray<double>&,
                                                            xt::xarray<double>&, xt::xarray<double>&,
                                                            xt::xarray<double>&, xt::xarray<double>&,
                                                            xt::xarray<double>&, xt::xarray<double>&);
...
#pragma omp parallel for
  for (int i = 0; i < num_coils; i++) {
    if (compute_dB)
      biot_savart_vjp_kernel<Array, 1>(pointsx, pointsy, pointsz, gammas[i], dgamma_by_dphis[i], v, res_gamma[i],
                                       res_dgamma_by_dphi[i], vgrad, res_grad_gamma[i], res_grad_dgamma_by_dphi[i]);
    else
      biot_savart_vjp_kernel<Array, 0>(pointsx, pointsy, pointsz, gammas[i], dgamma_by_dphis[i], v, res_gamma[i],
                                       res_dgamma_by_dphi[i], dummy, dummy, dummy);
