...
def test_vmec_jax_discrete_backend_exposes_exact_residual_and_post_jacobian_callbacks():
    vmec = VmecJax(_input_filename(), verbose=False)
    vmec.indata.mpol = 3
    vmec.indata.ntor = 3
    vmec.set_solver_options(
    ...
    residual_exact = np.asarray(stage.residuals.scipy_residuals(x0))
    _ = np.asarray(stage.residuals.scipy_jacobian(x0))
    residual_after_jac = np.asarray(stage.residuals.scipy_exact_residual_after_jacobian())

    np.testing.assert_allclose(residual_after_jac, residual_exact)
    assert stage.residuals.scipy_state_payload(x0)[1] is not None

    stage.residuals.scipy_post_jacobian_callback()
    assert stage.residuals.scipy_exact_residual_after_jacobian() is None
