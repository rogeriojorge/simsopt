...
The top-level flow intentionally mirrors boozerQA.py: choose input/output
parameters, build the initial surface and volume label, solve the Boozer
surface, define the objective, write VTK output, optimize, and export final
artifacts.
"""
...
print(f"Wrote optimized Biot-Savart JSON to {driver.history['artifacts']['coils_json']}")
print(f"Wrote Boozer surface restart to {driver.history['artifacts']['surface_restart']}")
print(
    f"Wrote VMEC input to {driver.history['artifacts']['vmec_input']} "
    f"using phiedge={driver.history['artifacts']['vmec_phiedge']:.6e}"
)
print(
    f"Wrote VMEC export diagnostics to {driver.history['artifacts']['vmec_export_diagnostics']} "
    f"with relative_l2={driver.history['artifacts']['vmec_export_relative_l2']:.3e}"
)
print(f"Wrote VMEC export cross-section plot to {driver.history['artifacts']['vmec_export_cross_sections_plot']}")
print(f"Wrote VMEC export 3D plot to {driver.history['artifacts']['vmec_export_surface_plot']}")
